...
def main():
    ...
    parser.add_argument("-t", "--timeout", type=int, default=1, help="Ping/port timeout in seconds (default: 1)")
    parser.add_argument("-w", "--workers", type=int, default=100, help="Number of concurrent threads (default: 100)")
    parser.add_argument("-o", "--output", help="Save output to a file (format depends on --format)")
    parser.add_argument("-q", "--quiet", action="store_true", help="Suppress real-time output, show only final results")
    # Additional features
    parser.add_argument("--hostname", "-H", action="store_true", help="Perform reverse DNS lookup for each active host")
    parser.add_argument("--ports", "-p", help="Comma-separated list of ports to scan on active hosts (e.g., 22,80,443)")
    parser.add_argument("--format", choices=['text', 'json', 'csv'], default='text', help="Output format (default: text)")
    parser.add_argument("--arp", action="store_true", help="Use ARP scanning instead of ICMP ping (requires root and scapy)")
